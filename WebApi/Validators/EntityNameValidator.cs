...
public class EntityNameValidator : AbstractValidator<EntityNameDto>
{
    public EntityNameValidator()
    {
        ...
        RuleForEach(x => x.EntityNames).Must(entity => !string.IsNullOrEmpty(entity.EnglishName))
            .WithMessage("EnglishName must not be null or empty for each item in EntityNames");
    }
}